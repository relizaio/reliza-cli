...
- **--imagefile** - flag which sets absolute path to the file with image string or image k8s json (optional, either images string or image file must be provided). Default value: */resources/images*. Use *kubectl get po -n kube-system -o json | jq ".items[] | [{namespace:.metadata.namespace, pod:.metadata.name, status:.status.containerStatuses[]}]"* to obtain k8s json.
